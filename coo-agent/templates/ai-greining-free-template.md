# AI-GREINING (ÓKEYPIS) - TEMPLATE

**Order Type:** Free AI Analysis
**Order ID:** [AUTO-FILL]
**Customer:** [AUTO-FILL]
**Industry:** [AUTO-FILL]
**Generated:** [AUTO-FILL DATE]
**Delivery:** PDF (5-8 pages)

---

## MUNURINN Á GREININGUM

| Feature | AI-Greining (Ókeypis) | 30-Day Roadmap (49.900 ISK) |
|---------|----------------------|----------------------------|
| **Delivery time** | 5-10 mínútur | 24 klst |
| **Length** | 5-8 síður | 20-30 síður |
| **Analysis depth** | Yfirborðsleg | Djúp greining |
| **Opportunities** | 3-5 listi | 3-5 með ROI & details |
| **Implementation plan** | ❌ Nei | ✅ Já (30 dagar, step-by-step) |
| **Tools recommended** | ❌ Nei | ✅ Já (specific tools) |
| **ROI calculations** | ❌ Nei | ✅ Já |
| **Success metrics** | ❌ Nei | ✅ Já |
| **Support** | ❌ Nei | ✅ Email support |

---

## COVER PAGE

```
[LIORATECH LOGO]

AI GREINING FYRIR [COMPANY NAME]

3-5 Tækifæri til úrbóta með gervigreind

━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Prepared by: LioraTech ehf.
Date: [DATE]
Type: Ókeypis AI-greining
Project ID: [ORDER_ID]

lioratech.is
info@lioratech.is
```

---

## PAGE 1: INTRO

### Takk fyrir að prófa AI-greininguna okkar!

Þessi greining er byggð á þeim upplýsingum sem þú gafst okkur um [COMPANY_NAME].

**Hvað þú færð í þessari skýrslu:**
- ✅ Stutt yfirlit yfir núverandi stöðu
- ✅ 3-5 tækifæri til úrbóta með AI
- ✅ Forgangsröðun (hvað á að byrja á)
- ✅ Áætlað gildi hvers tækifæris

**Hvað þú færð EKKI:**
- ❌ Nákvæman implementation plan
- ❌ Tól og verkfæri tillögur
- ❌ ROI útreikninga
- ❌ Tímasetningar og milestones

*(Þetta er allt innifalið í 30-daga roadmap-inu)*

**Markmið þessarar greiningar:**
Gefa þér skýra mynd af því hvar AI getur skapað mest verðmæti í þínum rekstri.

---

## PAGE 2: CURRENT STATE (NÚVERANDI STAÐA)

### Um [COMPANY_NAME]

**Grunnupplýsingar:**
- Industry: [INDUSTRY]
- Team size: [TEAM_SIZE]
- Main challenge: [TOP_PAIN_POINT]

### Þín helstu áskoranir

Þú nefndir þessar áskoranir:

**1. [PAIN_POINT_1]**
- Þetta er algengt í [INDUSTRY] geiranum
- Áætlað tap: [TIME/MONEY] á viku/mánuði
- AI getur hjálpað: [Já/Nei/Að hluta]

**2. [PAIN_POINT_2]**
- Þetta er algengt í [INDUSTRY] geiranum
- Áætlað tap: [TIME/MONEY] á viku/mánuði
- AI getur hjálpað: [Já/Nei/Að hluta]

**3. [PAIN_POINT_3]**
- Þetta er algengt í [INDUSTRY] geiranum
- Áætlað tap: [TIME/MONEY] á viku/mánuði
- AI getur hjálpað: [Já/Nei/Að hluta]

---

## PAGE 3-6: TÆKIFÆRI (3-5 opportunities)

### 🥇 TÆKIFÆRI #1: [NAME]

**Hvað þetta er:**
[2-3 setningar sem útskýra tækifærið]

**Hvernig það hjálpar þér:**
- [Benefit 1]
- [Benefit 2]
- [Benefit 3]

**Áætlað gildi:**
- ⏰ Tímasparnaður: [X tímar/viku]
- 💰 Kostnaðarsparnaður: [Y ISK/mánuð]
- 📈 Gæðabót: [Improvement]

**Erfiðleikastig:**
🟢 Auðvelt | 🟡 Miðlungs | 🔴 Krefjandi

**Forgangsröðun:** ⭐⭐⭐⭐⭐ (5/5)

**Næstu skref ef þú vilt innleiða:**
1. [High-level step 1]
2. [High-level step 2]
3. [High-level step 3]

*(Nákvæm innleiðingaráætlun er í 30-daga roadmap)*

---

### 🥈 TÆKIFÆRI #2: [NAME]

[Same structure as #1]

**Forgangsröðun:** ⭐⭐⭐⭐ (4/5)

---

### 🥉 TÆKIFÆRI #3: [NAME]

[Same structure as #1]

**Forgangsröðun:** ⭐⭐⭐ (3/5)

---

### 💡 TÆKIFÆRI #4: [NAME] *(ef á við)*

[Same structure]

**Forgangsröðun:** ⭐⭐ (2/5)

---

### 💡 TÆKIFÆRI #5: [NAME] *(ef á við)*

[Same structure]

**Forgangsröðun:** ⭐ (1/5)

---

## PAGE 7: FORGANGSRÖÐUN

### Hvað áttu að gera fyrst?

Byggt á þinni stöðu, mælum við með að byrja hér:

**Skref 1: Innleiða [OPPORTUNITY #1]**
- Hvers vegna: [Reason]
- Áætlaður tími: [X dagar/vikur]
- Þú þarft: [Resources needed - high level]

**Skref 2: Innleiða [OPPORTUNITY #2]**
- Hvers vegna: [Reason]
- Áætlaður tími: [X dagar/vikur]
- Þú þarft: [Resources needed - high level]

**Skref 3: Meta árangur og halda áfram**
- Mæla árangur af #1 og #2
- Ákveða hvort halda áfram með #3
- Eða finna ný tækifæri

---

## PAGE 8: NÆSTU SKREF

### Hvað gerist núna?

Þú hefur séð tækifærin. Nú þarftu að velja hvernig þú vilt halda áfram.

---

**Option 1: Innleiða sjálfur** ✅ ÓKEYPIS

Þú getur notað þessa greiningu til að byrja sjálfur.

**Hvað það kostar:**
- 0 ISK fyrir þessa greiningu
- Þú þarft að finna tól, prófa þig áfram og byggja sjálfur
- Áætlaður tími: 3-6 mánuðir til að ná árangri

**Hæfir fyrir:**
- Þá sem hafa tíma og tæknilega færni
- Þá sem vilja læra og prófa sjálf
- Þá sem eru ekki með tímaþvingun

---

**Option 2: Fá nákvæman 30-daga framkvæmdaáætlun** 💎

### Þú hefur séð HVAÐ er hægt að gera. Viltu vita HVERNIG?

**Verð: 49.900 ISK** (einstaklingsbundið tilboð)

Persónuleg ráðgjöf með sérfræðingi + ítarleg 30 daga framkvæmdaáætlun:

**Hvað þú færð:**
- ✅ **Djúp greining** í samstarfi við þig (ekki bara sjálfvirk greining)
- ✅ **Nákvæmur 30 daga plan** – dag fyrir dag, skref fyrir skref
- ✅ **Specific tól og verkfæri** – nákvæmlega hvað þú þarft
- ✅ **ROI útreikninga** – sérð nákvæmlega hvað þú sparar
- ✅ **Success metrics** – vitum hvort við náum árangri
- ✅ **Forgangsröðun** – hvað á að gera fyrst og hvernig
- ✅ **Áhættumöt og lausnir** – við grípum vandamál áður en þau koma upp
- ✅ **Fagleg PDF skýrsla** – tilbúið til að deila með teyminu

**Delivery:** Tilbúið eftir 3-5 virka daga

**Af hverju 30 daga plan?**
- 📊 **Engin tilraun og villa** – þú veist nákvæmlega hvað virkar
- ⏱️ **Sparar 2-4 mánuði** miðað við að prófa þig áfram sjálfur
- 💰 **Hámarkar ROI** – við einblínum á það sem skilar bestum árangri
- 🎯 **Tilbúið til framkvæmda strax** – allt er í einum stað

**Hentar þér ef:**
- ⚡ Þú vilt ná árangri hratt (ekki eyða mánuðum í tilraunir)
- 💡 Þú vilt sérfræðiráðgjöf og skýra stefnu
- 📈 Þú vilt hámarkar ROI og forðast dýr mistök
- ⏰ Þú hefur ekki tíma til að prófa þig áfram sjálfur

👉 **[Panta 30 daga framkvæmdaáætlun](https://lioratech.is/30roadmap)**

---

**Option 3: Full innleiðing með okkur** 🚀

Við innleiðum allt fyrir þig:

**Þú færð:**
- ✅ **Allt úr 30 daga áætlun**
- ✅ Við byggjum alla sjálfvirkni fyrir þig
- ✅ Við setjum upp öll tól og tengingar
- ✅ Við þjálfum teymið þitt
- ✅ Viðvarandi support og fínstillingar

**Hentar þér ef:**
- 🚀 Þú vilt "done-for-you" lausn
- ⚡ Þú vilt ná árangri enn hraðar
- 🔧 Þú hefur ekki innri tæknilega getu

**Verð:** Custom (fer eftir umfangi)

👉 **[Fá verðtilboð](https://lioratech.is/quote)**

---

## CLOSING

### Takk fyrir að prófa LioraTech! 🎉

Þú hefur nú séð hvað er hægt. Spurningin er: **Hvenær byrjarðu?**

---

### Hvað ætlarðu að gera næst?

**Valmöguleikar þínir:**

1. **📋 Gera ekkert** – Greiningin situr í skúffu, tækifærin týnast, keppendur komast á undan
2. **🛠️ Prófa þig áfram sjálfur** – Getur tekið 3-6 mánuði, kostað dýr mistök, en er ókeypis
3. **🚀 Fá 30 daga framkvæmdaáætlun** – Nákvæm stefna, sparar 2-4 mánuði, forðast mistök → **[Panta hér](https://lioratech.is/30roadmap)**
4. **💎 Full innleiðing með okkur** – Done-for-you lausn, hraðasti leiðin → **[Fá tilboð](https://lioratech.is/quote)**

---

**Spurningar?**
- Email: ingi@lioratech.is
- Website: lioratech.is

**Meira efni:**
- Vikubréf með AI ráðleggingum: [Skrá sig](https://lioratech.is#newsletter)

---

**Gangi þér vel með AI innleiðinguna!**

Ingi Þór Gunnarsson
AI Ráðgjafi & Innleiðingarsérfræðingur
LioraTech ehf.
ingi@lioratech.is

---

*Þessi greining var búin til með AI og er afhent eins og hún er. Fyrir ítarlegar tillögur og innleiðingarstuðning skaltu íhuga að panta 30 Daga Framkvæmdaáætlun.*

---

## TECHNICAL NOTES (for COO-Agent)

**Generation prompts for each section:**

### Industry Analysis Prompt:
```
Industry: {INDUSTRY}
Pain points: {PAIN_POINTS}

Generate:
1. What are common AI use cases in {INDUSTRY}?
2. What are competitors doing?
3. What is realistic for a team of {TEAM_SIZE}?
Keep it brief (2-3 sentences max).
```

### Opportunity Identification Prompt:
```
Company: {COMPANY_NAME}
Industry: {INDUSTRY}
Pain points: {PAIN_POINTS}
Team size: {TEAM_SIZE}
Main goal: {MAIN_GOAL}

Generate 3-5 AI opportunities ranked by:
- ROI potential
- Implementation difficulty
- Time to value

For each opportunity, provide:
1. Name (short, clear)
2. Description (2-3 sentences)
3. Benefits (3 bullet points)
4. Estimated value (time saved, cost saved, quality improvement)
5. Difficulty level (Easy/Medium/Hard)
6. Priority score (1-5)
7. Next steps (3 high-level steps)

Make it actionable but not too detailed (that's the paid product).
```

### Prioritization Prompt:
```
Given these opportunities: {OPPORTUNITIES}

Recommend:
1. Which to start with and why
2. Which to do second and why
3. What to measure

Keep it simple and clear.
```

**Delivery time target:** 5-10 minutes (fully automated)
**Format:** PDF, 5-8 pages, branded with LioraTech colors
**Automation level:** 95% (human only reviews if flagged by quality check)
